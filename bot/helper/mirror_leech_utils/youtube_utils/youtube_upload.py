            ...
            if os.path.isfile(self._path):
                ...
                if upload_mode == "playlist":
                    if provided_playlist_id:
                        self.playlist_id = (
                            provided_playlist_id  # Ensure _upload_video uses it
                        )
                        individual_video_urls = []
                        files_processed = 0
                            ...
                            if os.path.isfile(item_path):
                                ...
                                if mime_type and mime_type.startswith("video/"):
                                    try:
                                        ...
                                        LOGGER.error(
                                            f"Failed to upload {item_name} to playlist {provided_playlist_id}: {e}"
                                        )
                                    if self.listener.is_cancelled:
                                        LOGGER.info(
                                            "Upload process cancelled during playlist video upload."
                                        )
                                        break
                        if not self.listener.is_cancelled:
                            upload_result_url = {
                                "playlist_url": f"https://www.youtube.com/playlist?list={provided_playlist_id}",
                                "individual_video_urls": individual_video_urls,
                            }
                            upload_type = "Playlist"
                            self.total_files = files_processed
                            LOGGER.info(
                                f"Uploaded videos to existing playlist ID: {provided_playlist_id}"
                            )
                        else:  # Handle cancellation
                            upload_result_url = None
                            files_processed = (
                                0  # Or count partially uploaded if needed
                            )
                    else:  # No provided_playlist_id, create new playlist
                        upload_result_url = self._upload_playlist(
                            self._path, playlist_name
                        )
                        upload_type = "Playlist"
                        if upload_result_url and upload_result_url.get(
                            "playlist_url"
                        ):
                            files_processed = (
                                self.total_files
                            )  # self.total_files is updated in _upload_playlist
                        else:
                            files_processed = 0
                        LOGGER.info(
                            f"Uploaded new Playlist To YouTube: {self._path}"
                        )
                elif upload_mode == "individual":
                    # This part remains largely the same as it doesn't involve playlists directly
                    self.playlist_id = (
                        None  # Ensure no playlist is used for individual uploads
                    )
                    individual_video_urls = []
                    files_processed = 0
                    self.total_files = 0
                        ...
                        if os.path.isfile(item_path):
                            ...
                            if mime_type and mime_type.startswith("video/"):
                                try:
                                    ...
                                    LOGGER.error(
                                        f"Failed to upload {item_name}: {e}"
                                    )
                                if self.listener.is_cancelled:
                                    LOGGER.info(
                                        "Upload process cancelled during individual video upload."
                                    )
                                    break
                    if not self.listener.is_cancelled:
                        upload_result_url = {
                    ...
                    if provided_playlist_id:
                            ...
                            if os.path.isfile(item_path):
                                ...
                                if mime_type and mime_type.startswith("video/"):
                                    try:
                                        ...
                                        LOGGER.error(
                                            f"Failed to upload {item_name} to playlist {provided_playlist_id} (mode: p&i): {e}"
                                        )
                                    if self.listener.is_cancelled:
                                        LOGGER.info(
                                            "Upload process cancelled during playlist_and_individual video upload."
                                        )
                                        break
                        if not self.listener.is_cancelled:
                            upload_result_url = {
                            ...
                            self.total_files = files_processed
                            LOGGER.info(
                                f"Uploaded videos to existing playlist ID {provided_playlist_id} (mode: p&i)"
                            )
                        else:
                            upload_result_url = None
                            files_processed = 0
                        ...
                        if upload_result_url and upload_result_url.get(
                            ...
                            files_processed = 0
                        LOGGER.info(
                            f"Uploaded new Playlist and Individual Videos To YouTube: {self._path}"
                        )
                else:
