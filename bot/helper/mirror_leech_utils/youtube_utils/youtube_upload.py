...
class YouTubeUpload(YouTubeHelper):
    ...
    def upload(self):
        ...
        self.user_setting()
        try:
            self.service = self.authorize(self.listener.user_id if hasattr(self.listener, 'user_id') else "")
        except Exception as e:
            LOGGER.error(f"YouTube authorization failed: {e}")
            async_to_sync(self.listener.on_upload_error, f"YouTube authorization failed: {str(e)}")
            return
        ...
        while response is None and not self.listener.is_cancelled:
            try:
                ...
                if err.resp.status in [500, 502, 503, 504, 429] and retries < 5:
                    retries += 1
                    LOGGER.warning(f"HTTP error {err.resp.status}, retrying... ({retries}/5)")
                    continue
                else:
                    error_content = err.content.decode('utf-8') if err.content else "Unknown error"
