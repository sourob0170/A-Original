...
class RcloneTransferHelper:
    ...
    async def _start_download(self, cmd, remote_type):
        ...
        if return_code != -9:
            ...
            if "ftpstream:" in error and "504 File not found" in error:
                ...
                if "ftpstream:" in error:
                    try:
                        ...
                        file_name = (
                            error.split("ftpstream:")[1].split("'")[0].strip()
                        )
                    except Exception:
                        pass
            ...
            if (
                "megavideo:" in error
                and "Failed to link: PublicLink failed to create link: Access violation"
                in error
            ):
                # Extract the file name from the error message for better error reporting
                file_name = "unknown file"
                if "megavideo:" in error:
                    try:
                        # Try to extract the filename from the error message
                        file_name = (
                            error.split("megavideo:")[1].split("'")[0].strip()
                        )
                    except Exception:
                        pass
