import unittest
from unittest.mock import patch, Mock, MagicMock, call, AsyncMock # Ensure AsyncMock is imported
...
class TestTaskListener(unittest.IsolatedAsyncioTestCase): # Use IsolatedAsyncioTestCase for async methods
    ...
    def common_mocks(self, listener_instance):
        ...
        listener_instance.is_file = True # Assume it's a file by default after processing
        ...
        listener_instance.is_super_chat = False # For INCOMPLETE_TASK_NOTIFIER related logic
        ...
        listener_instance.proceed_extract = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.proceed_watermark = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.proceed_metadata = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.proceed_ffmpeg = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.substitute = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.remove_www_prefix = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.generate_screenshots = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.convert_media = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.generate_sample_video = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.proceed_compress = AsyncMock(return_value=f"{listener_instance.dir}{listener_instance.name}")
        listener_instance.proceed_split = AsyncMock()
        listener_instance.on_upload_error = AsyncMock() # To check if it's called on uploader config error
    ...
    @patch('bot.helper.listeners.task_listener.aiopath.exists', new_callable=AsyncMock, return_value=True)
    @patch('bot.helper.listeners.task_listener.get_path_size', new_callable=AsyncMock, return_value=1000)
    @patch('bot.helper.listeners.task_listener.aiopath.isfile', new_callable=AsyncMock, return_value=True)
    @patch('bot.helper.listeners.task_listener.listdir', new_callable=AsyncMock, return_value=['test_task_name'])
    @patch('bot.helper.listeners.task_listener.remove_excluded_files', new_callable=AsyncMock)
    @patch('bot.helper.listeners.task_listener.update_status_message', new_callable=AsyncMock)
    @patch('bot.helper.listeners.task_listener.check_running_tasks', new_callable=AsyncMock, return_value=(False, Mock(wait=AsyncMock())))
    @patch('bot.helper.listeners.task_listener.start_from_queued', new_callable=AsyncMock)
    @patch('bot.helper.listeners.task_listener.task_dict', MOCK_TASK_DICT) # Patch task_dict
    async def run_on_download_complete_with_mocks(self, listener, mock_start_queued, mock_check_running, mock_update_status, mock_remove_excluded, mock_listdir, mock_isfile, mock_get_path_size, mock_exists):
        # Store a mock download object in the task_dict for the listener to retrieve
        MOCK_TASK_DICT[listener.mid] = MockDownload(name=listener.name, gid=listener.mid)
    ...
    async def test_uploader_yt_selected(self, MockYouTubeUpload):
        ...
        MockYouTubeUpload.assert_called_once_with(listener, f"{listener.dir}{listener.name}")
    ...
    async def test_uploader_gd_selected_valid_path(self, MockGoogleDriveUpload, mock_is_gdrive_id):
        listener = await self.create_listener_instance()
        ...
        MockGoogleDriveUpload.assert_called_once_with(listener, f"{listener.dir}{listener.name}")
    ...
    @patch('bot.helper.listeners.task_listener.GoogleDriveUpload')
    async def test_uploader_gd_selected_invalid_path(self, MockGoogleDriveUpload, mock_is_gdrive_id):
        listener = await self.create_listener_instance()
        listener.uploader = "gd"
        ...
        listener.on_upload_error.assert_called_once_with(f"Google Drive ID not provided or invalid in -up / UPSTREAM_REPO for -ul gd. Path: {listener.up_dest}")
    ...
    async def test_uploader_rc_selected_valid_path(self, MockRcloneTransferHelper):
        ...
        MockRcloneTransferHelper.return_value.upload.assert_called_once_with(f"{listener.dir}{listener.name}")
    ...
    async def test_uploader_rc_selected_no_path(self, MockRcloneTransferHelper):
        ...
        listener.on_upload_error.assert_called_once_with("Rclone destination not provided in -up / UPSTREAM_REPO for -ul rc.")
    ...
    @patch('bot.helper.listeners.task_listener.is_gdrive_id', return_value=True)
    @patch('bot.helper.listeners.task_listener.GoogleDriveUpload')
    async def test_fallback_gdrive_from_up_dest(self, MockGoogleDriveUpload, mock_is_gdrive_id):
    ...
    async def test_fallback_rclone_from_up_dest(self, MockRcloneTransferHelper):
        with patch.object(Config, 'DEFAULT_UPLOAD', ""): # No default uploader
            with patch('bot.helper.listeners.task_listener.is_gdrive_id', return_value=False): # -up is not GDrive
                listener = await self.create_listener_instance()
                listener.uploader = "" # No -ul
    ...
    async def test_fallback_leech(self, MockTelegramUploader):
        with patch.object(Config, 'DEFAULT_UPLOAD', ""): # No default uploader
            ...
            MockTelegramUploader.assert_called_once_with(listener, listener.dir) # Leech uploads from base dir
