...
class TestYouTubeUpload(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_listener.message.chat.id = (
            "chat_id"  # Mock chat_id for status updates
        )
        ...
        self.youtube_upload = YouTubeUpload(
            listener=self.mock_listener, path=self.test_path
        )
        ...
        self.patcher_set_interval = patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.SetInterval"
        )
        self.mock_set_interval = self.patcher_set_interval.start()

        self.patcher_logger = patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.LOGGER"
        )
        self.mock_logger = self.patcher_logger.start()

        # Mock async_to_sync which is used for listener callbacks
        self.patcher_async_to_sync = patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.async_to_sync"
        )
        self.mock_async_to_sync = self.patcher_async_to_sync.start()

        # Mock get_mime_type
        self.patcher_get_mime_type = patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.get_mime_type"
        )
        self.mock_get_mime_type = self.patcher_get_mime_type.start()

        # Mock os.remove
        self.patcher_os_remove = patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.remove"
        )
    ...
    def test_upload_playlist_successful(
        self, mock_os_isfile_item, mock_os_join, mock_os_listdir, mock_os_isdir
    ):
        ...
        def isfile_side_effect(path):
            return path in [
                "/fake/folder/video1.mp4",
                "/fake/folder/video2.mkv",
                "/fake/folder/notes.txt",
            ]

        mock_os_isfile_item.side_effect = isfile_side_effect

        self.mock_get_mime_type.side_effect = (
            lambda path: "video/mp4"
        ...
        with patch(
            "bot.helper.mirror_leech_utils.youtube_utils.youtube_upload.MediaFileUpload"
            ...
            mock_playlist_insert().execute.return_value = {
                "id": "pl123",
                "kind": "youtube#playlist",
            }

            # Mock adding items to playlist
            mock_playlist_item_insert = (
                self.youtube_upload.service.playlistItems().insert
            )
            mock_playlist_item_insert().execute.return_value = {
                "id": "pli1",
        ...
        self.assertEqual(
            self.youtube_upload.total_files, 2
        )  # Check total_files set in _upload_playlist
    ...
    def test_upload_folder_no_video_files(
        self, mock_os_isfile_item, mock_os_join, mock_os_listdir, mock_os_isdir
    ):
        self.youtube_upload._path = "/fake/no_videos_folder"
        self.mock_get_mime_type.return_value = (
            "text/plain"  # All files are non-video
        )
    ...
    def test_upload_single_video_file_successful(
        self, mock_os_isdir, mock_os_isfile
    ):
        ...
        with patch.object(
            self.youtube_upload,
            "_upload_video",
            return_value="https://www.youtube.com/watch?v=vid123",
        ) as mock_internal_upload_video:
            self.youtube_upload.upload()

            mock_internal_upload_video.assert_called_once_with(
                "/fake/video.mp4", self.mock_listener.name, "video/mp4"
            )
