...
def get_cloudflare_speedtest_results():
    ...
    try:
        ...
        result = subprocess.run(
            ['python', '-c', 'from speedtest_cloudflare_cli.main import main; main()', '--json', '--download', '--upload'],
            capture_output=True,
            text=True,
        ...
        try:
            ...
            dict_part = re.sub(r'datetime\.datetime\([^)]+\)', '"datetime_object"', dict_part)
            ...
            download_speed = f"{download_data.get('speed', 0):.2f} Mbps" if download_data.get('speed') else "Test failed"
            upload_speed = f"{upload_data.get('speed', 0):.2f} Mbps" if upload_data.get('speed') else "Test failed"

            # Use download latency if available, otherwise use http_latency
            ping_latency = download_data.get('latency') or download_data.get('http_latency', 0)
...
async def speedtest(_, message):
    ...
    await edit_message(status_msg, "🔄 <b>Running Standard Speedtest...</b>\n<i>Testing with nearest server...</i>")

    standard_result = await TgClient.bot.loop.run_in_executor(None, get_standard_speedtest_results)
    ...
    if standard_result:
        ...
        standard_speed_text += f"• <b>Ping:</b> <code>{standard_result.ping:.2f} ms</code>\n"
        ...
        standard_speed_text += f"• <b>IP Address:</b> <code>{standard_result.client['ip']}</code>\n"
        standard_speed_text += f"• <b>ISP:</b> <code>{standard_result.client['isp']}</code>\n"
        standard_speed_text += f"• <b>Server:</b> <code>{standard_result.server['name']}</code>\n"
        standard_speed_text += f"• <b>Location:</b> <code>{standard_result.server['name']}, {standard_result.server['country']}</code>\n"
        standard_speed_text += f"• <b>Distance:</b> <code>{standard_result.server['d']:.2f} km</code>"

        # Send standard speedtest results with photo
        try:
            await send_message(message, standard_speed_text, photo=standard_result.share())
    ...
    await edit_message(status_msg, "☁️ <b>Running Cloudflare Speedtest...</b>\n<i>Testing with Cloudflare servers...</i>\n<i>⏱️ This may take several minutes for slow connections...</i>")

    # Run Cloudflare speedtest with fallback
    cf_result = await TgClient.bot.loop.run_in_executor(None, get_cloudflare_speedtest_results)

    # If Cloudflare test failed, try fallback
    if not cf_result:
        await edit_message(status_msg, "🔄 <b>Cloudflare test failed, running fallback speedtest...</b>\n<i>Using speedtest.net servers...</i>\n<i>⏱️ This may take several minutes...</i>")
        cf_result = await TgClient.bot.loop.run_in_executor(None, get_fallback_speedtest_results)
    ...
    if cf_result:
        ...
        cf_speed_text += f"• <b>Ping (Latency):</b> <code>{cf_result['ping']}</code>\n"

        if cf_result['jitter'] != "N/A":
            cf_speed_text += f"• <b>Jitter:</b> <code>{cf_result['jitter']}</code>\n"

        if cf_result['http_latency'] != "N/A":
            cf_speed_text += f"• <b>HTTP Latency:</b> <code>{cf_result['http_latency']}</code>\n"

        cf_speed_text += f"• <b>Download:</b> <code>{cf_result['download_speed']}</code>\n"
        cf_speed_text += f"• <b>Upload:</b> <code>{cf_result['upload_speed']}</code>\n\n"
        ...
        if cf_result['hostname'] != "N/A":
            cf_speed_text += f"• <b>Server:</b> <code>{cf_result['hostname']}</code>\n"

        if cf_result['protocol'] != "N/A":
            cf_speed_text += f"• <b>Protocol:</b> <code>{cf_result['protocol']}</code>\n"

        if cf_result['timestamp'] != "N/A":
            cf_speed_text += f"• <b>Test Time:</b> <code>{cf_result['timestamp'][:19]}</code>"
    else:
